...
$C_{p}=\frac{Müşteri \ toleransı}{Doğal \ tolerans}=\frac{USL-ASL}{6 \sigma}$
...
$C_{pk}=min[C_{palt},C_{püst}]=min[\frac{\mu-ASL}{3\sigma},\frac{USL-\mu}{3\sigma}]$
...
Hedef değer (bu problem için 490 gr.), ASL ve ÜSL'nin tam ortasında ve proses ortalaması hedef değere eşit olduğunda $C_{p}=C_{pk}$   olur ve proses merkezileşmiştir. $C_{p}$ ve  $C_{pk}$ için aşağıdaki çıkarımlar yapılabilir;
